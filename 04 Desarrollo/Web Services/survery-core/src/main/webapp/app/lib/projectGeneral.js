project = {};
project.utils = {};
project.utils.constants = function(){
	var Virtual_Directory = '/survery';
	return {
		
		URL_PROJECT 					: Virtual_Directory + '/',
		URL_SERVICE_REQUEST 			: Virtual_Directory + '/', 
		URL_IMAGE 						: Virtual_Directory + '/resources/images/',
		
		regexNormalizeName 				: new RegExp("[A-Za-z0-9_^Ññ]"),
		regexTextNumSpace 				: new RegExp("[A-Za-z0-9\\s]"), 
		regexTextNum 					: new RegExp("[A-Za-z0-9]"), 
		regexTextSpace 					: new RegExp("[A-Za-z\\s]"),
		regexText 						: new RegExp("[A-Za-z]"),
		regexNumDecimal 				: new RegExp("[0-9.]"),
		regexDateFormat 				: new RegExp("[0-9\/]"),
		regexDateTimeFormat 			: new RegExp("[0-9\/:.\\s]"),
		regexDateFormatDefinition		: new RegExp("[\|AMDamd\/-\\s]"), 
		regexDateTimeFormatDefinition 	: new RegExp("[\|AMDTHISamdthis\/-\\s:.]")
	};
} ();
...
(function ($) {
    ...
    $.fn.extend({
        ...
        createStaticFlash: function(type, title, message) {
        	this.createFlashMessage({
        		type: type,
                message: (title != null && title != '' ? '<b>'+title+'</b> ' : '' ) + message,
                closeable: true,
                permanent: true,
                position: 'static'
        	});
